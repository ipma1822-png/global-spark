...
begin
 ...
 with member_stats as (
  select m.id,m.display_name,m.member_code,
   ...
   coalesce((select k.active from public.spark_kid_links k where k.member_id=m.id limit 1),false) link_active,
   (select k.last_used_at from public.spark_kid_links k where k.member_id=m.id limit 1) link_last_used
  from public.spark_members m where m.center_id=v_center_id and m.active=true
 )
 select jsonb_build_object('active_members',count(*),'active_today',count(*) filter(where last_activity>=date_trunc('day',now())),'active_7d',count(*) filter(where last_activity>=now()-interval '7 days'),'inactive_7d',count(*) filter(where last_activity is null or last_activity<now()-interval '7 days'),'xp_7d',coalesce(sum(xp_7d),0),'self_today',coalesce(sum(self_today),0),'self_7d',coalesce(sum(self_7d),0),'self_members_7d',count(*) filter(where self_7d>0),'active_kid_links',count(*) filter(where link_active),'attention_members',count(*) filter(where last_activity is null or last_activity<now()-interval '7 days' or self_today>=4)),
 coalesce(jsonb_agg(jsonb_build_object('id',id,'display_name',display_name,'member_code',member_code,'total_xp',total_xp,'xp_7d',xp_7d,'activity_7d',activity_7d,'last_activity',last_activity,'dominant_flame',dominant_flame,'repeat_7d',repeat_7d,'self_today',self_today,'self_7d',self_7d,'link_active',link_active,'link_last_used',link_last_used,'needs_attention',(last_activity is null or last_activity<now()-interval '7 days' or self_today>=4),'attention_reason',case when self_today>=4 then '직접기록 많음' when last_activity is null or last_activity<now()-interval '7 days' then '7일 미활동' else null end) order by display_name),'[]'::jsonb)
