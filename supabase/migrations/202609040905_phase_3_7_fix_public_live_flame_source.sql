...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object('center_name',name,'country_code',country_code,'region_name',region_name,'today_count',today_count,'week_count',week_count,'total_xp',total_xp) order by today_count desc,week_count desc),'[]'::jsonb) into v_center_rows from (select c.id,c.name,c.country_code,coalesce(c.region_name,'') region_name,(select count(*) from spark_activities a where a.center_id=c.id and a.created_at>=date_trunc('day',now()) and exists(select 1 from spark_ledger l where l.activity_id=a.id group by l.activity_id having sum(l.amount)>0)) today_count,(select count(*) from spark_activities a where a.center_id=c.id and a.created_at>=now()-interval '7 days' and exists(select 1 from spark_ledger l where l.activity_id=a.id group by l.activity_id having sum(l.amount)>0)) week_count,(select coalesce(sum(l.amount),0) from spark_ledger l where l.center_id=c.id) total_xp from spark_centers c where c.status in ('pilot','active') order by today_count desc,week_count desc limit 20)q;
  with codes(code) as (values ('GOOD'),('SAFE'),('EARTH'),('CHALLENGE'),('CITIZEN')) select jsonb_object_agg(code,cnt) into v_flames from (select codes.code,coalesce(x.cnt,0) cnt from codes left join (select coalesce(r.flame_code,'GOOD') code,count(*) cnt from spark_activities a left join spark_activity_rules r on r.activity_type=a.activity_type where a.created_at>=now()-interval '7 days' and exists(select 1 from spark_ledger l where l.activity_id=a.id group by l.activity_id having sum(l.amount)>0) group by coalesce(r.flame_code,'GOOD'))x on x.code=codes.code)z;
