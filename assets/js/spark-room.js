...
(async function(){
  ...
  function renderGrowth(total,level){
    ...
    $('character').src=stage.asset+'?v=270';
  ...
  if(!kidToken){$('hello').textContent='개인 링크가 필요해요';$('welcome').textContent='관장님이 보내준 내 SPARK 링크로 들어와 주세요.';$('activitySection').classList.add('hidden');$('recent').innerHTML='<p class="help">개인 링크로 들어오면 내 점수와 기록이 보여요.</p>';return}
  ...
  $('submit').onclick=async()=>{
    ...
    try{
      const r=await SparkData.registerKidActivity(kidToken,selected,$('memo').value.trim());
      const row=Array.isArray(r)?r[0]:r;
      setStatus(`🎉 바로 반영됐어요! +${Number(row?.xp||0)} XP`);
      $('memo').value='';selected='';await reload();
    }catch(e){console.error(e);setStatus('기록하지 못했어요. 링크가 맞는지 관장님께 알려 주세요.',true)}
    finally{btn.disabled=!selected}
  };
  try{await reload();setStatus('오늘 한 일을 하나 골라 주세요.')}catch(e){console.error(e);$('hello').textContent='불꽃방을 열 수 없어요';$('welcome').textContent='링크가 만료되었거나 아직 준비되지 않았어요.';$('activitySection').classList.add('hidden');$('recent').innerHTML='<p class="help error">관장님께 새 개인 링크를 받아 주세요.</p>'}
})();
